...
public class GameCollControl : MonoBehaviour
{
	...
	public static float Spd = 0.5f;
	public static float Spdt = 0.3f;
	public static float Spdspeed = 0.3f;
...
/*	//Проверка достигла ли игра 1 усложнения
	...
	public void Addhardcoll()
	{
		...
		if ( BalloonScoreColl >= sixhard ) 
		{
			Time.timeScale = 2.5f;
		}
		if ( BalloonScoreColl >= sevenhard )
		{
			Time.timeScale = 2.7f;
		}
		if ( BalloonScoreColl >= eighthard )
		{
			Time.timeScale = 2.9f;
		}
		if ( BalloonScoreColl >= ninehard )
		{
			Time.timeScale = 3.1f;
		}
		if ( BalloonScoreColl >= tenhard )
		{
			Time.timeScale = 3.3f;
		}
	} 
